...
def _normalize_type_name(t):
    ...
    #burayi degistir her bi seye uyarla xd
def get_test_files(test_type=None):

    inputs_dir = os.path.join(TESTCASE_ROOT, "inputs")
    if not os.path.exists(inputs_dir):
        return []

    input_files = sorted(glob.glob(os.path.join(inputs_dir, "*.txt")))

    if test_type:
        prefix = test_type.strip().lower() + "_"
        filtered = []
        for f in input_files:
            base = os.path.basename(f).lower()
            if base.startswith(prefix):
                filtered.append(f)
        input_files = sorted(filtered)
...
def run_single_test(input_file, verbose=False, benchmark=False):
    ...
    # Expected file: testcases/outputs/<basename>_out.txt
    outputs_dir = os.path.join(TESTCASE_ROOT, "outputs")
...
def main():
    parser = argparse.ArgumentParser(description="hw3_runner")
